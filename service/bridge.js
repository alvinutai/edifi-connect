...
const { detectOpenDental } = require("../electron-app/od-detect");
const {
...
const {
  decideSyncCommandStatus,
} = require("../electron-app/lib/sync-command-status");
...
async function sendAgentHello() {
  ...
  log(
    `[RemoteControl] AGENT_HELLO sent: v${"2.3.61-service"} caps=${AGENT_CAPABILITIES.length}`,
  );
}
...
async function handleReportUpdateStatus(commandId) {
  const result = {
    current_version: "2.3.61-service",
    ...update_status,
  };
...
async function syncODData(syncDate = null) {
  ...
  // Refresh the coverage category cache once for the whole window — dates after
  // the first reuse the warmed cache.
  odCovCatCache = null;
...
async function syncODMySql(syncDate) {
  ...
  try {
    ...
    // DB/query failure fails the whole window — report it, don't fall through
    // as an empty "success".
    od_sync_status.last_error = e.message.slice(0, 200);
